...
let print_tyvar_list (l : (tyvar * mode) list) =
  List.iter (fun (a, m) -> match a with Var str -> print_string (str ^ " - " ^ (string_of_mode m))) l;;
...
let rec update_def (d : def) (l : (tyvar * mode) list) =
  match d with
    Def (tym, name, bod, mode) -> Def ((List.map (fun (a, b) ->  
                                            ...
                                            then let c = List.assoc a l in (* if not (mode_inc c b)
                                                                           then *) (a, c)
                                                                                  (*   else (a, b) *)
...
let rec check_type (tocheck : ty) (defs : def list) (m : mode) =
  ...
  if m = Ind then [] else
    match tocheck with
    | Unit | Int | Float | Bool | Char -> let _ = (print_string "\n1\n") in  []
    | Arrow (a, b) | Cons (a, b) | Or (a, b) ->  let _ = (print_string "\n2\n") in 
                                                 let outa = (check_type a defs (mode_point m Ind)) in
                                                 let outb = (check_type b defs (mode_point m Ind)) in
                                                 List.append outa outb
    | Tyvar a ->  let _ = (print_string "\n3\n") in let _ = print_tyvar_list ((a, m) :: []) in [(a, m)]
    | Param (par, name) ->  let _ = (print_string "\n4\n") in  check_args par defs (get_ty_mode name defs)
    | Exists (ex, bod) ->  let _ = (print_string "\n5\n") in  begin
    ...
    | Abstract par ->  let _ = (print_string "\n6\n") in  []
...
and check_def (tocheck : def) (defs : def list) (m : mode) =
  ...
  match tocheck with
    Def (partc, nametc, tytc, modetc) ->
     match tytc with
     | Unit | Int | Float | Char |Bool -> (* let _ = (print_string "\n01\n") in *) []
     | Arrow (_, _) | Cons (_, _) | Or (_, _) -> (* let _ = (print_string "\n02\n") in *) (check_type tytc defs (mode_point m Ind))
     | Param (_, _) | Exists (_, _) | Abstract _  -> check_type tytc defs m
     | Tyvar a -> let out = check_type tytc defs m in subsection (fun (x, y) -> let assoc = (List.assoc x partc) in ((mode_inc assoc y) && not (mode_inc y assoc))) out
  ...
  let rec aux (todo : def list) (defs : def list) =
    ...
    | hd :: tl ->
       begin
         ...
         | [] -> aux tl defs
         | a -> let _= print_tyvar_list a in let _ = print_string " ----- " in let _ = match hd with Def (x, _, _, _) -> print_tyvar_list x in aux defs (replace_def defs hd (update_def hd a))
       end
  in aux defs defs;;
...
let recursive_1 = ("type ('a, 'b) t = 'a \nand ('c, 'd) u = ('c, 'd) t", [
                     ...
                     Def ([(Var "c", Ind); (Var "d", Ind)], Name "u", Param ([Tyvar (Var "c"); Tyvar (Var "d")], Name "t"), Sep)], true,
                   [[(Var "a", Sep); (Var "b", Ind)];
                    [(Var "c", Sep); (Var "d", Ind)]])

let tree = ("type ('a, 'k) tree = bool -> 'a node \nand 'b node = N : ('b, 'c) tree", [
              Def ([(Var "a", Ind); (Var "k", Ind)], Name "tree", Arrow (Bool, Param ([Tyvar (Var "a")], Name "node")), Sep);
              Def ([Var "b", Ind], Name "node", Exists (Var "c", Param ([Tyvar (Var "b"); Tyvar (Var "c")], Name "tree")), Sep)], true, [
                [(Var "a", Ind); (Var "k", Ind)];
                [(Var "b", Ind)]
           ])
...
let inclus = ("type 'a t : ind and u = bool t", [Def ([(Var "a", Ind)], Name "t", Unit, Ind);
                                                 Def ([], Name "u", Param ([Bool], Name "t"), Ind)], true, [
                  [(Var "a", Ind)]])
...
let test (case : test) =
  let aux (defs : def list) (par : (tyvar * mode) list list) =
    let outpar = List.flatten (List.map (fun (Def (pars, _, _, _)) -> pars) defs) in
     let _ = List.iter (fun (Var v, m) -> print_string (v ^ " / " ^ (string_of_mode m))) outpar in 
    List.for_all2 (fun (_, m1) (_, m2) -> mode_inc m1 m2) outpar (List.flatten par) in
  let success str = print_string (str ^ "   -   test passed\n") in
  let failure str = print_string (str ^ "   -   test failed\n") in
  match case with
    (str, defs, pass, expected) ->
    try let out = (check defs) in if pass && (aux out expected) then success str else failure str
    with Existential_is_not_sep e -> (*let _ = print_string "notsep" in*) if pass then failure str else success str;;
